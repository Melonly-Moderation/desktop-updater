...
module.exports = class Cache {
  ...
  async logReleaseCheck(version) {
    await fetch(process.env.RELEASE_CHECK_WEBHOOK_URL, {
      headers: {
        ...
        embeds: [
          {
            ...
            author: {
              name: 'release-check',
              icon_url: 'https://melonly.xyz/brand/logo_with_whitespace.png',
            },
            description: `Fetched latest releases, ${version ? `new version: ${version}`: 'no new releases.'}`,
            footer: {
  ...
  async refreshCache() {
    ...
    if (this.latest.version === tag_name) {
      ...
      await this.logReleaseCheck();
      return
    }

    await this.logReleaseCheck(tag_name);
